...
Each ``__getitem__`` returns ONE full sequence:
  tactile : (T, 64, 64) float32  — every frame in the sequence
...
from velocity_dataLoader_batch import _compute_dt, _load_sequence_timestamps
...
class sample_velocity_seq_data(Dataset):
    ...
    def __getitem__(self, seq_idx):
        ...
        tactile = np.empty((T, 64, 64), dtype=np.float32)
        for i in range(T):
...
def collate_pad(batch):
    ...
    T_max = max(lengths)
    B = len(batch)
    tac = np.zeros((B, T_max, 64, 64), dtype=np.float32)

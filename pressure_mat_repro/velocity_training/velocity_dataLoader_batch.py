...
def window_select(log, path, f, idx, window):
    if window == 0:
        ...
        return np.reshape(data[0], (1, 96, 96)), np.reshape(data[0], (1, 96, 96))
    ...
    tactile_frame = np.reshape(data_center[0], (1, 96, 96))
    tactile = np.empty((2 * window, 96, 96), dtype=np.float32)

...
from velocity_dataLoader_seq_32_noisy import sample_velocity_seq_data, collate_pad
...
parser.add_argument("--noise_sigma", type=float, default=0.0,
                    help="Gaussian sigma (Newtons) added to tactile per __getitem__.")
...
def make_loader(path, shuffle, noise_sigma=0.0):
...
if __name__ == "__main__":
    ...
    train_loader = make_loader(args.train_dir, shuffle=True, noise_sigma=args.noise_sigma)
    val_loader = make_loader(args.val_dir, shuffle=False, noise_sigma=0.0)  # clean val

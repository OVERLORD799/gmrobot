...
HOME_POSITION = np.array([0.9, 0.0, 0.4], dtype=np.float32)  # GMRobot original
_DESK_Z = 0.0  # GMRobot original
...
GRASP_MAX_REWIND_ATTEMPTS = 5  # R7: was 2 — too low for virtual-hand STOP testing
...
class SingleEnvPickAndPlacePolicy:
    ...
    def __init__(self):
        ...
        self._any_carry_aborted = False   # R7 H3 fix: latch — never cleared, used by success check
    ...
    def _build_stage_sequence(self, obs):
        ...
        for command in self.user_commands:
            ...
            pick_pos[-1] = _DESK_Z  # container base z (was 0.0 in GMRobot)
            ...
            place_pos[-1] = _DESK_Z  # container base z (was 0.0 in GMRobot)
    ...
    def on_replan_splice_applied(self, task_step: int) -> None:
        ...
        R2 M5 fix: never clear state while a grasp rewind is in progress — doing so
        would reset _grasp_disturbance_pending and _grasp_rewind_force_open, causing
        the robot to skip grasp validation on the re-approach after the detour.
        """
        if self._grasp_rewind_force_open:
            return
        ...
        if (
            ...
            return
        # R7: don't reset _grasp_rewind_attempts on replan — that creates
        # an infinite loop of rewind→replan→rewind→replan.  Only clear the
        # disturbance pending flag; the rewind counter persists across replans.
        self._grasp_disturbance_pending = False
        self._grasp_disturbance_step = -1
        self._grasp_rewind_force_open = False
        self._stabilize_hold_steps = 0
    ...
    def trigger_vlm_retry_current_part(self) -> bool:
        ...
        if self._vlm_retry_count >= VLM_MAX_RETRIES:
            self._grasp_rewind_force_open = False  # R3 M5 fix: clear stale flag
            return False
        part_idx = self.part_index_at_step(self.time_step)
        if part_idx is None:
            return False
    ...
    def maybe_rewind_for_failed_grasp(
        ...
        if self._grasp_rewind_attempts >= GRASP_MAX_REWIND_ATTEMPTS:
            ...
            self._any_carry_aborted = True   # R7 H3 fix: latch for success check
            self.clear_grasp_disturbance()
            return False
    ...
    def transport_phase_at_step(self, step: int) -> str:
        ...
        if not self._grasp_hold_validated and (
            ...
            return "approach"
        # R3 H3 fix: block replan during pick-side lift when grasp is
        # unvalidated — the EE is rising from a potentially empty pick.
        if not self._grasp_hold_validated and self._grasp_disturbance_pending:
            if name.startswith("lift_") and not name.startswith("lift_after_releasing_"):
                return "approach"
        return "transit"
    ...
    def _compute_rejoin_step(self, at_step: int, detour_duration: int) -> int:
        ...
        # R7: was 3* — rejoined too close to blocked point, causing loop.
        # 8* skips past the blocked zone so UR10e rejoins at a clear segment.
        naive = min(at_step + 8 * detour_duration, traj_end)
    ...
    def advance_time_step(self) -> None:
        ...
        if self.time_step >= self.time_stamps[-1]:
            # R7 H3 fix: don't report success if any part was lost during carry.
            # _any_carry_aborted latches to True and never resets — it catches
            # knock-offs that happened earlier in the episode, not just the last one.
            if not self._any_carry_aborted:
                self.success = True
    ...
    def splice_replan_detour(
        ...
        *,
        ...
        # R3 H1/H4 fix: also guard against forward jumps that would skip
        # stage transitions (close_gripper clears _grasp_rewind_force_open,
        # open_gripper_to_release clears _grasp_hold_validated).  A detour
        # should always inject at or before the current clock position.
        if at_step < self.time_step - 1 or at_step > self.time_step:
            return False
        ...
        # R3 M1 fix: reset the grasp stabilize hold so it re-fires at the
        # correct move_above pose after the detour, not at offset waypoints.
        if self._stabilize_hold_steps > 0:
            self._stabilize_hold_steps = GRASP_STABILIZE_HOLD_STEPS
    ...
    def _build_detour_waypoints(
        ...
        *,
        ...
        # R7 raise_high: go OVER the obstacle entirely then descend to target.
        # Used when headroom > Z_HEADROOM_RAISE_HIGH_M (0.25m).  Raises EE
        # above the hand's operating envelope, translates laterally, then
        # descends to a point above the container before rejoining.
        if detour_strategy == "raise_high":
            high_z = max(ee_pos[2] + raise_m, 0.80)  # at least 0.80 m
            high = ee_pos.copy()
            high[2] = high_z
            high_lateral = lateral_fn(high, human_hand_pos, lateral_m)
            high_lateral[2] = high_z
            descend = high_lateral.copy()
            descend[2] = max(rejoin_pos[2] + 0.08, ee_pos[2])
            descend = _maybe_clamp(descend)
            return [
                ("replan_detour_raise_high", high),
                ("replan_detour_high_lateral", high_lateral),
                ("replan_detour_descend", descend),
                ("replan_detour_rejoin", rejoin_pos.copy()),
            ]

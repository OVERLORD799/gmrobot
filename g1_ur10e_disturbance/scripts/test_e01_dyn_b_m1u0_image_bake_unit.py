...
def test_build_context_sources_contain_outer_lateral_patrol():
    ...
    assert all(rel in flags for rel in M1U0_BAKE_FILES)


def test_canonical_smoke_no_pip_prebundle_and_no_network_models():
    cmd = canonical_dyn_b_smoke_shell(
        output_csv="/opt/projects/g1_ur10e_disturbance/results/paper_demo/v1m1u0_dyn_b_image_smoke_20260723/safety_logs/phase3.csv",
        numpy_origin_json="/opt/projects/g1_ur10e_disturbance/results/paper_demo/v1m1u0_dyn_b_image_smoke_20260723/meta/numpy_origin.json",
    )
